...
V1.4.6 FALLBACK PROPERTIES FIX
------------------------------
- Fixes the issue where sample/fallback properties disappeared after selecting India or another country.
- Adds country/location fields to fallback properties.
- Changes frontend public cache key to bgpPublicDataCacheV146 to avoid old cached empty data.
- If backend returns zero public properties, the website keeps showing fallback/sample properties instead of a blank listing page.
- To show real backend properties, make sure Properties rows have PublicStatus=Show, VerificationStatus=Verified, and Status=Listed or Active.


V1.4.5 DEBUG COUNTS FAST FIX
----------------------------
- debugCounts is now a fast diagnostic route.
- backend-test.html runs fewer backend checks.
- Do not use debugCounts for public website loading.
- If sheet headers need repair, run setupSheets() manually in Apps Script, then deploy a new Web App version.
...
V1.4.3 Dashboard Portals update
--------------------------------
This version adds improved Customer, Seller, and Admin dashboard portals.

1. Customer Dashboard
   - URL: customer-dashboard.html
   - Customers can view profile, buyer requirements, property enquiries, referrals, and pending reward count.
   - Customer login now creates a CustomerToken saved in the Customers sheet.
   - Existing customers should login again after deployment to generate the new token.

2. Seller Dashboard
   - Hidden direct URL: seller-login.html
   - Dashboard URL: seller-dashboard.html
   - Sellers can view submitted property status, pending/published count, enquiries linked to their property codes, and site visit/deal progress.
   - Buyer contact details are still not shown to sellers to preserve the no-direct-dealing mediator model.
   - Sell Property form now includes optional seller dashboard password creation.

3. Hidden Admin Dashboard
   - Hidden direct URL: admin-login.html
   - Dashboard URL: admin-dashboard.html
   - Admin can view counts, pending property approvals, enquiries, buyer requirements, referrals, proposals, and deals.
   - Detailed editing, approval, publishing, proposal generation, and print/archive actions remain in Google Sheets + Apps Script.
   - Default MVP PIN: 123456.
   - Change it before launch in Settings sheet: AdminDashboardPIN.

Deployment after this update
----------------------------
1. Replace all website files on GitHub Pages with this package.
...
5. Confirm js/config.js contains the latest Web App /exec URL.
6. Open backend-test.html and run tests.
7. Open customer-login.html, seller-login.html, and admin-login.html for portal testing.
8. Because the service worker cache version changed to bgp-pwa-v1-4-3, hard refresh or reinstall the PWA if old pages appear.

Security note
-------------
This is still a low-cost Google Sheets + Apps Script MVP. For full commercial launch, upgrade admin/seller/customer authentication to Firebase Auth, Supabase Auth, or another secure server-side auth system.


============================================================
V1.4.4 SPEED OPTIMIZATION NOTE
============================================================
This version reduces slow backend loading by:
1. Combining public Countries + Banners + Properties into one Apps Script action: publicData.
2. Showing cached/sample public data immediately instead of waiting on Google Apps Script.
3. Refreshing backend data in the background when the backend responds.
4. Removing full setupSheetsIfMissing() execution from every public read request.
5. Adding a frontend timeout message for very slow Apps Script responses.
6. Adding a Clear Frontend Cache button in backend-test.html.

After replacing files:
- Replace apps-script/Code.gs in Google Apps Script.
- Run setupSheets() once manually from Apps Script.
- Deploy a NEW Web App version.
- Upload all website files, especially js/main.js and service-worker.js.
- Clear browser/PWA cache once or reinstall the PWA.
- Open backend-test.html and check "Public Data Combined".

Recommended Google Sheet hygiene for speed:
- Keep only approved/live listings in Properties; archive old/closed records.
- Avoid very large image URLs or long notes in public listing rows.
- Use direct compressed image URLs for banners and properties.
- Do not keep thousands of blank formatted rows in Google Sheets.
