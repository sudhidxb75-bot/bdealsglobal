/*
  Bdeals Global Properties - Frontend Script V1.4.6
  ...
  V1.4.6 fallback update:
  - Sample fallback properties now include country/location fields so country filters do not hide them
  - Property filters allow fallback/sample records if country is missing
  - Empty backend property lists keep showing fallback records instead of blank results
*/
...
const SAMPLE_PROPERTIES = [
  ...
  {
    propertyCode: "BGP-1004",
    propertyType: "Apartment",
    country: "AE",
    countryName: "United Arab Emirates",
    location: "Dubai",
    locality: "Dubai Residential Area",
    area: "Dubai",
    sizeText: "1 to 2 BHK options",
    publicPriceRange: "Price on verified request",
    description: "Sample UAE property option for fallback display. Replace with verified backend listings before public launch.",
    highlights: "Family building, Parking, Easy access",
    imageURL: "",
    featured: "No",
    sampleFallback: "Yes"
  },
  {
    propertyCode: "BGP-1005",
    ...
    area: "Dubai",
    sizeText: "Office / retail option",
    publicPriceRange: "Price on verified request",
    description: "Sample commercial property option for fallback display. Full details are shared only after verification.",
    highlights: "Commercial location, Road access, Business use",
    imageURL: "",
    featured: "No",
    sampleFallback: "Yes"
  }
...
const BGP_PUBLIC_CACHE_KEY = "bgpPublicDataCacheV146";
...
function getInitialPublicProperties() {
...
function isUsingSampleFallbackProperties(list) {
  return Array.isArray(list) && list.length && list.every(item => String(item.sampleFallback || item.SampleFallback || "").toLowerCase() === "yes");
}
...
async function loadPublicProperties() {
  allProperties = getInitialPublicProperties();
  ...
  try {
    ...
    if (data.ok && Array.isArray(data.properties) && data.properties.length) {
      ...
      // Backend is connected but has no public properties yet.
      // Keep visible fallback listings instead of showing a blank page.
...
function renderProperties(targetId, list) {
  ...
  const fallbackNotice = isUsingSampleFallbackProperties(list) && targetId === "propertyList" ? `
    <div class="policy-card fallback-note" style="grid-column:1/-1;padding:16px 20px;background:#fff7ed;border:1px solid #fed7aa">
      <strong>Showing fallback/sample properties.</strong> Backend is connected but no approved public property listings are available yet, or backend data is still loading.
    </div>
  ` : "";

  target.innerHTML = fallbackNotice + list.map(property => {
...
function applyPropertyFilters() {
  ...
  const filtered = allProperties.filter(property => {
    ...
    const isSample = String(property.sampleFallback || property.SampleFallback || "").toLowerCase() === "yes";
    const countryMatches = !country || countryText.includes(country) || (!countryText && isSample);
...
function normalizeProperty(property) {
  const selectedCountry = localStorage.getItem("bgpSelectedCountry") || "IN";
  return {
    ...
    country: safe(property.country || property.Country || (String(property.sampleFallback || property.SampleFallback || "").toLowerCase() === "yes" ? selectedCountry : "")),
    countryName: safe(property.countryName || property.CountryName || (selectedCountry === "IN" ? "India" : selectedCountry === "AE" ? "United Arab Emirates" : "")),
    location: safe(property.location || property.Location || (String(property.sampleFallback || property.SampleFallback || "").toLowerCase() === "yes" ? "Calicut" : "")),
...
function initBackendTestPage() {
  ...
  box.innerHTML = `
    <div class="dashboard-card">
      ...
      <div class="hero-actions"><button class="btn primary" onclick="runBackendTests()">Run Backend Tests</button><button class="btn secondary" onclick="clearBGPFrontendCache()">Clear Frontend Cache</button></div><p class="form-note">V1.4.5 runs fewer backend tests and uses fast debug counts. Run setupSheets() manually only after backend changes.</p>
...
async function runBackendTests() {
  ...
  const tests = [
    ...
    ["Debug Counts Fast", () => apiGet("debugCounts", {}, { timeoutMs: 30000 })]
  ];
